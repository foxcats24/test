...
class Pm extends Model
{
 ...
 public function getByAlias($id_user_recipient){
    ...
    public function getToByAlias($id_user_sender){
      ...
      public function getRecipient(){ //view
        $params = App::getRouter()->getParams(); 
        $id_user_recipient=Session::get('userid'); 
        $id_pm_chat = strtolower($params[0]);
        return $this->db->query('SELECT id_user_sender FROM chats_pm WHERE id_user_recipient=?', $id_user_recipient);
        
 }
 public function getChatList($id_user_sender, $id_user_recipient){ //view
      
    return $this->db->query('SELECT private_messages.message, private_messages.id, users.id, users.login  FROM private_messages INNER JOIN users  ON private_messages.id_user_sender=users.id OR n private_messages.id_user_recipient=? WHERE  id_user_sender=?', $id_user_recipient, $id_user_sender);
